...
class LGNLIF(nengo.neurons.NeuronType):
    ...
    def step_math(self, dt, J, spiked, voltage, threshold):        
        ...
        ###
        H = spiked * dt # new
        U = np.matmul(H, self.Ss) + y # np.matmul(self.S_x, y)
        ...
        spiked[:] = spiked_mask * (self.amplitude) / dt # new: put divide by dt

        # set spiked voltages to v_reset
