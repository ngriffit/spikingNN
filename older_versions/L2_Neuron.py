...
class LGNLIF(nengo.neurons.NeuronType):
    ...
    def step_math(self, dt, J, spiked, voltage, refractory_time, threshold):        
        ...
        refractory_time -= dt
#         print("B neuron step" + str(self.counter))
#         print("BEFORE UPDATE")
#         print("voltage:")
#         print(voltage)
#         print("current:")
...
#         print("this is np.matmul(spiked, self.Ss):{}".format(np.matmul(spiked, self.Ss)))
        # step voltage
        # J is currently weights * spiked from previous layer
        # now we want to do relu by only letting the values greater than thresh continue

        y = np.maximum(J - self.thresh, 0) # ReLU activation function 
        win, maxInd = np.max(y), np.argmax(y)
#         print("this is winner value {}".format(win))
        # LGN competition rule
        y[y < win] = 0 # only allow winner to participate (make all other entries 0)
#         print("THIS IS Y: {}".format(y))

        U = np.matmul(spiked, self.Ss) + y # np.matmul(self.S_x, y)
        # should use S_x, but S_x is too negative rn for some unknown rsn
#         U = np.matmul(spiked, self.Ss) + np.matmul(self.Ss, y)

#         print("number of values in U that are +: {}".format((U > 0).sum()))
...
#         print("voltage after stepmath")
#         print(voltage)

        
from nengo.builder.operator import Operator

class SimLGNLIF(Operator):
...
@Builder.register(LGNLIF)
def build_LGNLIF(model, neuron_type, neurons):
